...
export const getAllContacts = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!contacts) {
      const err = new Errors("No contacts found", 404, "failed")
      next(err)
      return
    }
    res.status(200).json({
    ...
    const err = new Errors(error.message, 500, "failed")
    next(err)
  }
}
...
export const getContactById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!contact) {
      const err = new Errors("Contact not found", 404, "failed")
      next(err)
      return
    }
    res.status(200).json({
...
export const addContact = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!errors.isEmpty()) {
      const err = new Errors(errors.array()[0].msg, 400, "failed")
      next(err)
      return
    }
    ...
    if (data.phone === dup?.phone) {
      const err = new Errors("Phone number already exist", 400, "failed")
      next(err)
      return
    }
    data.save()
